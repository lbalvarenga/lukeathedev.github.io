...
class Board {
  ...
  constructor(fenString, style) {
    _defineProperty(this, "fenString", "");

    _defineProperty(this, "tileImage", null);

    _defineProperty(this, "currentTurn", Piece.sides.white);

    _defineProperty(this, "canCastle", {
      "K": false,
      // White king side
      "Q": false,
      // White queen side
    ...
    _defineProperty(this, "isInCheck", {
      "white": false,
      "black": false
    });
  ...
  async validateMove(src, dst, promotionHandler) {
    ...
    switch (srcPiece.type) {
      case Piece.types.pawn:
        let side; // TODO: implement en passant

        if (srcPiece.side == Piece.sides.white) side = 1;
        if (srcPiece.side == Piece.sides.black) side = -1; // En Passant target
    ...
    } else this.currentTurn = Piece.sides.white; // Determine if side is in check
    // Find active king
  ...
  listAllMovesLegal(side) {
    ...
    for (let y = 0; y < sz; y++) {
      for (let x = 0; x < sz; x++) {
        ...
        moves = moves.concat(this.listMovesLegal(piece, {
          "x": x,
          "y": y
        }));
      }
  ...
  listChecks(kingPos, side) {
    let moves = [];
    let sz = this.style.size;
    let kingPiece = this.tiles[kingPos.y][kingPos.x]; // This is a mess right now
    ...
    let angles = [-135, -90, -45, 0, 45, 90, 135, 180]; // BUG: kingPiece.side causes moves not on your turn to not show

    let attAngles = Utils.rayCastC(kingPos, angles, this.tiles, 8, side); // for each cast ray at angle
    ...
    for (let i = 0; i < attAngles.length; i++) {
      ...
      if (attPiece.length < 1 || attMoves.length < 1) continue; // List attacking piece moves

      let piecePos = {
        "x": attPiece[0] % 8,
        "y": floor(attPiece[0] / 8)
      };
      let pieceAttacks = this.listMovesPseudo(this.tiles[piecePos.y][piecePos.x], piecePos); // Make sure it can reach king at some point
  ...
  listMovesLegal(piece, pos) {
    ...
    let moves = this.listMovesPseudo(piece, pos); // List pseudo moves
    ...
    for (let i = 0; i < moves.length; i++) {
      ...
      this.tiles[pos.y][pos.x] = new Piece(Piece.types.empty, null, null); // Test if king gets checked

      let possibleChecks = this.listChecks(kingPos, kingPiece.side);
      if (possibleChecks.length > 0) delete moves[i]; // Set them back
    ...
    if (piece.type == Piece.types.king) {
      kingPos = this.getPiecePos(Piece.types.king, piece.side)[0]; // Queen side
    ...
    switch (piece.type) {
      case Piece.types.pawn:
        ...
        if (piece.side == Piece.sides.white) {
          side = -1; // If at starting position
        ...
        for (let i = 0; i < moves.length; i++) {
          ...
          let x = moves[i] % 8;
          let y = floor(moves[i] / 8); // Check for pieces on diagonal
  ...
  readFen() {
    ...
    for (const c of this.fenString) {
      ...
      if (pos + row * 8 > this.style.size * this.style.size - 1) {
        ...
        if (turn == " w ") turn = Piece.sides.white;else if (turn == " b ") turn = Piece.sides.black;
      ...
      switch (c.toLowerCase()) {
        ...
        case "/":
          if (pos % this.style.size != 0) {// the FEN is most likely bad, but for
  ...
  writeFen() {
    ...
    this.fenString += this.currentTurn == Piece.sides.white ? " w" : " b"; // White castling

    this.fenString += this.canCastle.K ? " K" : " -";
    this.fenString += this.canCastle.Q ? "Q" : "-"; // Black castling
    ...
    if (this.enPassantTarget.x == null || this.enPassantTarget.y == null) {
      ...
      let enPassantAlgebraic = Utils.toAlgebraic(new Piece(Piece.types.empty, null, null), this.enPassantTarget, this.enPassantTarget, false);
